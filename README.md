...
1. **Batch 6 — close independent structural verification and engineering QA (in progress).** The genuine Stage-5 STAAD benchmark, engineering source/model review, finalized v1 tolerances and the analysis-vs-design acceptance matrix are implemented. Five scoped JRC reference checks now cover LM1 characteristic constants, the road-traffic gamma/frequent-factor basis, EC2 concrete-only shear, rectangular flexure and vertical-link spacing. The road-traffic ULS default has been corrected from 1.50 to the JRC bridge reference 1.35, and the former hardcoded L/1000 road-bridge deflection limit has been removed in favour of an explicit project/client criterion with frequent SLS as the default assessment combination. Broad milestones deliberately remain locked until LM1 lane/placement logic, the full EN 1990 bridge combination matrix, link-governed shear and representative flexure/torsion/cracking/deflection/fatigue/detailing evidence are complete; construction-stage and local-deck structural checks also remain external-verification items. MIDAS cross-verification is deferred to V2.
