...
- Batch 6P raises the suite to **eighteen** independent checks and closes the MSc serviceability calculation gates. European Concrete Platform Example 7.3 is reproduced with the production layered crack kernel, including compression reinforcement: **x=237.86 mm**, **I_II=5.957×10^9 mm⁴**, **σ_s=234.29 MPa** and **w_k=0.18487 mm**, matching the published 237.8 mm / 5.96×10^9 mm⁴ / 234 MPa / 0.184 mm results. Example 7.6 is reproduced after replacing span-wide ζ interpolation with **spatially varying EC2 curvature interpolation** along the actual service moment diagram: state-I deflection is **21.64 mm** and the cracked/uncracked result is about **35.8 mm** versus the published **35.71 mm**. This closes the simple-span MSc **crack-width** and **deflection-calculation** gates; the allowable road-bridge deflection remains an explicit project/client criterion rather than a universal Eurocode limit
- Batch 6Q closes the **explicitly scoped four-target MSc deterministic verification profile** without promoting the wider standalone application to full-v1 acceptance. The production layered positive-bending steel-sizing path now carries an explicit **z ≤ 0.95d** EC2 design lever-arm cap and reproduces The Concrete Centre heavily loaded L-beam reference (**MEd=1148 kNm**, **d=668 mm**, native **As,req≈4161 mm²** versus published **4158 mm²**; six H32 bars give **≈4825 mm²** versus published nominal **4824 mm²**). A separate 15 m FLM3 longitudinal benchmark reproduces the independent **936 kNm** midspan moving-vehicle moment/range for the standard four 120 kN axle lines. The native production LM1 grillage search is regression-audited for the full independent two-lane tandem Cartesian candidate space, lane plus remaining-area UDL coverage, both transverse edge layouts and governing M/V/T/service-deflection recovery. A convergence-controlled production mode now halves the longitudinal movement step until the worst M/V/T/deflection envelope change is within the configured tolerance; reduced tandem searches cannot self-certify convergence. `VerificationScope.MSC_SIMPLE_SPAN_ANN` now maps only the accepted simple-span Eurocode evidence needed by the current ANN targets (**g_M, g_V, g_crack, g_deflection**); the legacy/full-application `ann_ready` gate remains conservative and false while broader items are open.

- **Stage 7 MSc scope lock has started.** The research context is frozen around the 15.0 m simple span, 14.95 m physical precast girder length, 11.0 m deck, 7.0 m carriageway, seven girders at 1.70 m spacing, 75 mm non-composite false slab and 175 mm participating in-situ slab. The current serviceability research criterion is **L/250**, giving **60 mm** for the 15 m span, consistent with JRC Eurocode worked-example/training material; it is not labelled as a universal normative EN 1992-2 bridge limit. The seven core ANN/RBDO inputs are now explicitly separated into **design variables** (girder width b, girder depth h, longitudinal steel area As) and **random variables** (fck, fyk, permanent-action multiplier lambda_G, traffic-action multiplier lambda_Q). Numerical design bounds and unresolved load-distribution parameters remain intentionally unfrozen, and a machine-readable guard prevents pilot/full dataset generation until those items are justified.
...
- 400 mm × 950 mm rectangular precast girder
- 14.95 m physical precast girder length, stored separately from the 15.0 m analysis span
...
The reference project is stored in `examples/reference_bridge_15m.py` and is now directly runnable. The physical precast member is a **400 × 950 mm rectangular beam**; after the 175 mm in-situ slab hardens and participates longitudinally, the production section builder derives the resulting **composite T-section** automatically. The 75 mm precast false slab remains weight-only by default. The 14.95 m fabrication length is retained as physical metadata and is not silently substituted for the 15.0 m analysis span.
...
4. **Independent structural validation — split by scope.** The primary Stage-5 simple-span benchmark has genuine STAAD `.ANL` numerical evidence, an accepted source/model-equivalence review, a finalized evidence-based tolerance policy and an explicit acceptance matrix. The **MSc simple-span four-target profile** now has scoped accepted evidence for LM1 loading/search, the permanent+LM1 combination core, transverse distribution, positive-bending layered flexure, shear, cracking and deflection. The **general standalone-v1 profile remains in progress**: the wider EN 1990 thermal/wind/pedestrian/braking/LM2 matrix, actual-project torsion-cell basis, fatigue-specific transverse stress histories, broad detailing, construction-stage and local-deck independent checks still remain. **MIDAS cross-verification is explicitly deferred to V2 and is not a v1 release gate.**
...
6. **Verification-manifest unlock — scoped.** The conservative full-application manifest remains locked. The simple-span Eurocode MSc workflow has a separately named, opt-in `MSC_SIMPLE_SPAN_ANN` scope for the current four ANN targets; no continuous-span or BS 5400 profile inherits that evidence. Large-scale dataset generation still follows the research input-space definition step below.
...
Training-data export requires a `DeterministicSolverVerification` for the exact solver profile being used. A single Boolean cannot unlock ANN data. Two named verification scopes now prevent unrelated maturity claims from being conflated:

- `FULL_APPLICATION_V1` retains the conservative all-feature gate: traffic loading, load combinations, flexure, shear, cracking, deflection, fatigue, detailing, transverse distribution, independent benchmarking and torsion when required.
- `MSC_SIMPLE_SPAN_ANN` is an explicit opt-in scope for the current four ANN targets only: **g_M, g_V, g_crack and g_deflection** under the accepted simple-span Eurocode permanent+LM1 research path. It requires accepted traffic loading, the scoped permanent+LM1 combination core, positive-bending layered flexure, shear, cracking, deflection, transverse distribution and the independent Stage-5 benchmark. It does **not** relabel fatigue, torsion, broad detailing, construction-stage/local-deck response or the wider standalone action matrix as verified.

The legacy `ann_ready` property intentionally continues to mean the full-application gate and therefore remains false while those broader items are open. MSc dataset callers must request `VerificationScope.MSC_SIMPLE_SPAN_ANN` explicitly; the multi-limit export also rejects projects outside the one-span simply-supported Eurocode/composite-slab scope and requires an explicit project/client deflection criterion before `g_deflection` can be exported.

The simple-span Eurocode, continuous-span Eurocode and BS 5400 solver profiles are distinct. Verification of one profile cannot certify records generated by another. The continuous Eurocode and BS 5400 research profiles remain locked until their own independent verification programmes are complete.
...
1. **Batch 6 — finish the remaining general-v1 verification items.** The genuine Stage-5 STAAD benchmark, source/model review and evidence-based tolerance policy are accepted. The published/reference suite contains **twenty scoped independent checks** after Batch 6Q. For the **current MSc four-target simple-span ANN profile**, LM1 loading/search, the permanent+LM1 combination core, transverse distribution, positive-bending layered flexure, shear, crack-width and deflection-calculation evidence are explicitly mapped into a separate machine-readable verification scope. The broader standalone application still requires the actual bridge torsion-cell `Ak/uk/tef` basis, fatigue-specific transverse distribution/stress histories, broad drawing/detailing verification, wider EN 1990 action combinations, construction-stage response and local-deck independent checks. MIDAS cross-verification remains deferred to V2.
2. **Batch 7 — harden and release deterministic application v1.0.** Complete GUI consistency/validation/error-handling review, report pagination/polish, project-file compatibility, packaging/versioning and final Windows release checks. The automated 15 m application acceptance smoke now exercises save/reopen, native LM1, extended actions, design/detailing, FLM3 workflow, report generation and STAAD/MIDAS governing-case export while relying on the already reviewed genuine STAAD evidence for external-response acceptance.
3. **Keep verification scopes explicit.** `FULL_APPLICATION_V1` remains conservative and is not unlocked by the MSc evidence. `MSC_SIMPLE_SPAN_ANN` is an explicit opt-in scope only for the current **g_M, g_V, g_crack, g_deflection** simple-span Eurocode research outputs and requires an in-scope project plus an explicit project/client numerical deflection criterion.
4. **Define the research input space.** Establish justified ANN/RBDO random-variable distributions, bounds, correlations, sampling assumptions, limit-state targets and feature schemas before large-scale dataset generation. Expansion into flexure regimes outside the verified positive-bending/compression-block-in-flange domain requires additional independent evidence.
5. **Generate and validate the research models.** After the input-space definition is frozen inside the verified deterministic domain, produce the deterministic dataset, train/validate the ANN surrogate, then complete reliability analysis and RBDO.
