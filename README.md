...
The reference project is stored in `examples/reference_bridge_15m.py`. It intentionally does **not** invent missing physical T-girder flange/web dimensions. Native grillage/application analysis requires a complete rectangular/T/I girder profile; until the research girder's flange/web dimensions are explicitly defined, the exact research reference remains a geometry/material benchmark rather than a runnable full-grillage project. The automated end-to-end application smoke therefore uses the runnable 15 m application baseline with the same 15.0 m span, 11.0 m deck, 7.0 m carriageway, seven girders at 1.70 m spacing and a fully specified physical girder profile.
