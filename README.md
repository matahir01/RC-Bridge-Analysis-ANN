...
1. **Batch 6 — close independent structural verification and engineering QA (in progress).** The genuine Stage-5 STAAD benchmark, engineering source/model review, finalized v1 tolerances and the analysis-vs-design acceptance matrix are implemented. The published-reference suite now contains **fifteen scoped independent checks** covering LM1 characteristic constants, lane subdivision, transverse tandem placement/distribution, the road-traffic gamma/frequent-factor basis, the MSc permanent+LM1 ULS/SLS combination core, the full simple-span EC2 shear equation path (V_Rd,c, required/minimum/provided links, V_Rd,s and V_Rd,max), rectangular flexure, vertical-link spacing, reinforcement/concrete fatigue, and the core torsion reinforcement/resistance/interaction equations. The **shear verification milestone is now externally accepted** for the MSc simple-span profile. The remaining MSc-critical gates are narrower: longitudinal LM1 governing-placement/envelope search, representative T/I/layered flexure, actual bridge torsion-cell Ak/uk/tef traceability, cracking, deflection, FLM3 structural stress-range generation and remaining detailing rules. The standalone application's wider EN 1990 thermal/wind/pedestrian/braking/LM2 matrix, construction-stage and local-deck structural checks remain general-v1 verification items; MIDAS cross-verification is deferred to V2.
