...
2. **Complete drawing-level detailing.** Native simple-span section-by-section link spacing and anchorage-extended longitudinal curtailment now retain a full-span minimum continuous-bar core. Explicit anchorage coefficient policies support project-approved straight/hooked/other geometries without inventing geometry factors; optional end-length checks, non-overlapping staggered lap groups and longitudinal cage-depth constructability checks are implemented. Drawing-level torsion-cage placement remains the main open detailing item.
