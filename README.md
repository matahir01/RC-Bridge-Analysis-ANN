...
3. **Complete the remaining fatigue scope.** Dedicated native full-width FLM3 now envelopes auditable left/right-packed notional-lane centre candidates when no physical fatigue-lane centre is supplied, while preserving explicit project/NA lane selection when known. Per-girder co-located moment and shear ranges are recovered. Simple-span rectangular/T/I production fatigue covers longitudinal reinforcement and concrete compression, plus optional vertical-link fatigue using the actual provided A_sw/s and an explicit link fatigue category. The link stress range currently uses a conservative full-truss cyclic-shear model. Local deck reinforcement/concrete fatigue can now be checked from a dedicated plate/slab or verified external local stress source, while native global-grillage-to-local-slab stress generation remains intentionally blocked. A full-length continuous-grillage FLM3 traffic search is also implemented and retains signed station-by-station M/V ranges for every girder; top/bottom continuous RC fatigue section design is intentionally deferred to the continuous production stage. Native local plate stress generation, clause-by-clause verification and final independent validation remain outstanding.
