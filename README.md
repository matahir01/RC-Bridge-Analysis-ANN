...
1. **Batch 6 — close independent structural verification and engineering QA (in progress).** The genuine Stage-5 STAAD benchmark, engineering source/model review, finalized v1 tolerances and the analysis-vs-design acceptance matrix are implemented. The published-reference suite now contains **eight scoped JRC checks** covering LM1 characteristic constants, notional-lane subdivision, the road-traffic gamma/frequent-factor basis, EC2 concrete-only shear, rectangular flexure, vertical-link spacing, reinforcement fatigue and concrete-compression fatigue. The road-traffic ULS default is 1.35 and road-bridge deflection acceptance is project/client-defined with frequent SLS as the default assessment combination. The remaining MSc-critical independent gates are now narrower: LM1 response-maximising placement/enveloping, the full EN 1990 bridge combination matrix, link-governed shear, representative T/I/layered flexure, torsion, cracking, deflection, FLM3 structural stress-range generation and the remaining detailing rules; construction-stage and local-deck structural checks also remain external-verification items. MIDAS cross-verification is deferred to V2.
