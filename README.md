...
1. **Batch 6 — close independent structural verification and engineering QA (in progress).** The genuine Stage-5 STAAD benchmark, engineering source/model review, finalized v1 tolerances and the analysis-vs-design acceptance matrix are implemented. The published-reference suite now contains **ten scoped JRC checks** covering LM1 characteristic constants, lane subdivision, transverse tandem placement/distribution, the road-traffic gamma/frequent-factor basis, the MSc permanent+LM1 ULS/SLS combination core, EC2 concrete-only shear, rectangular flexure, vertical-link spacing, reinforcement fatigue and concrete-compression fatigue. The road-traffic ULS default is 1.35 and road-bridge deflection acceptance is project/client-defined with frequent SLS as the default assessment combination. The remaining MSc-critical independent gates are now narrower: the longitudinal LM1 governing-placement/envelope search, link-governed shear, representative T/I/layered flexure, torsion, cracking, deflection, FLM3 structural stress-range generation and remaining detailing rules. The standalone application's wider EN 1990 thermal/wind/pedestrian/braking/LM2 matrix, construction-stage and local-deck structural checks remain general-v1 verification items; MIDAS cross-verification is deferred to V2.
