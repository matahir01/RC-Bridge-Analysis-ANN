...
4. **Independent structural validation — in progress.** The primary Stage-5 simple-span benchmark now has genuine STAAD `.ANL` numerical evidence plus an accepted source/model-equivalence review. Deterministic v1.0 will close the remaining tolerance justification and clause/project acceptance items using the verified STAAD path. **MIDAS cross-verification is explicitly deferred to V2 and is not a v1 release gate.**
...
1. **Batch 6 — close independent structural verification and engineering QA (in progress).** The first genuine Stage-5 STAAD benchmark is numerically and engineering accepted, including source/model-equivalence review. For v1, the remaining work is to justify/finalize the project verification tolerances from genuine evidence, close outstanding clause/project acceptance checks, and make the verification manifest accurately reflect those decisions. MIDAS cross-verification is deferred to V2.
