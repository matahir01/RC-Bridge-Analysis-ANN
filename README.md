...
3. **Professional GUI completion — in progress.** The versioned engine/application interface is frozen at v1.0 and the professional sidebar/overview/calculation-review shell is in place. The next application work is deeper redesign of project inputs, graphical bridge/section views, analysis/result diagrams, design presentation, construction-stage review and verification UX.
4. **Independent structural validation — pending.** Run the application-generated verification files in installed STAAD.Pro and/or MIDAS Civil, import genuine results, reconcile axes/signs/units/model assumptions, establish justified tolerances and close clause/project acceptance gaps.
...
The installed-software MIDAS/STAAD comparison is a **final acceptance gate for the deterministic application**, not a substitute for internal mechanics/software tests. Synthetic CI fixtures and native round trips remain software verification only and must never be presented as independent structural validation.
...
6. **Desktop application workbench v2 is implemented; professional GUI refinement is in progress.** Project persistence, design-basis settings, load/action workspaces, native analysis, design/deck/fatigue workflows, shared calculation traces, HTML/PDF reports, performance caching, Stage-5 verification export/import, evidence generation, frozen engine/application interface v1.0 and the professional sidebar/overview/calculation-review shell are implemented. Further GUI work should refine presentation and interaction without rewriting validated engineering kernels.

## Remaining major work

1. **Batch 5 — complete the professional GUI.** Redesign the actual Project, Loads, Analysis, Design, Deck/Fatigue and Verification workspaces; add bridge/section visualisation, M/V/T/deflection diagrams, clearer governing-result cards, reinforcement presentation, construction-stage review and contextual **Show calculation** actions.
2. **Batch 6 — close independent structural verification and engineering QA.** Run the generated Stage-5 and supporting verification models in installed STAAD.Pro/MIDAS Civil, import genuine results, resolve discrepancies, confirm geometry/boundary/load/result-axis equivalence, justify numerical tolerances and complete outstanding clause/project acceptance checks.
3. **Batch 7 — harden and release deterministic application v1.0.** Finish end-to-end workflow testing, GUI consistency, remaining performance issues, validation/error handling, report pagination and polish, project-file compatibility, documentation/help, EXE versioning and the final Windows release build.
4. **Complete verification manifests before ANN export.** Keep ANN ground-truth generation locked until the exact solver profile has genuine evidence for traffic loading, combinations, flexure, shear, cracking, deflection, fatigue, detailing, transverse distribution, independent benchmarking and torsion where required.
5. **Define the research input space.** Establish justified ANN/RBDO random-variable distributions, bounds, correlations, sampling assumptions, limit-state targets and feature schemas before large-scale dataset generation.
6. **Generate and validate the research models.** Produce verified deterministic datasets, train/validate the ANN surrogate, then complete reliability analysis and RBDO.
