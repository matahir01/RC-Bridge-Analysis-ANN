...
4. **Independent structural validation — in progress.** The primary Stage-5 simple-span benchmark now has genuine STAAD `.ANL` numerical evidence, an accepted source/model-equivalence review and a finalized evidence-based numerical tolerance policy. Deterministic v1.0 still needs the remaining clause/project acceptance matrix to distinguish what the STAAD analysis evidence proves from design-code checks that require their own equation/benchmark evidence. **MIDAS cross-verification is explicitly deferred to V2 and is not a v1 release gate.**
...
1. **Batch 6 — close independent structural verification and engineering QA (in progress).** The first genuine Stage-5 STAAD benchmark is numerically and engineering accepted, including source/model-equivalence review, and the v1 numerical tolerance policy is now finalized from the genuine ANL print resolution and observed differences. Remaining v1 work is to close the clause/project acceptance matrix and make the solver-profile verification manifest reflect exactly which analysis/design milestones have independent evidence. MIDAS cross-verification is deferred to V2.
