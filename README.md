...
1. **Batch 6 — close independent structural verification and engineering QA (in progress).** The application now parses and compares the full STAAD result catalogue — primary load cases and load combinations — while also performing the higher-level governing LM1 envelope checks for per-girder M/V/T, service deflection and permanent-action global equilibrium. Remaining work is to exercise this against genuine project files across the wider benchmark matrix, resolve any discrepancies, confirm geometry/boundary/load/result-axis equivalence, justify final tolerances and complete outstanding clause/project acceptance checks.
