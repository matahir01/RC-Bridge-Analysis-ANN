...
- Verification acceptance now uses one shared **relative + quantity-specific absolute tolerance** policy for detailed result checks and governing envelopes. Near-zero external verification quantities are judged by absolute error rather than receiving an automatic PASS when percentage error is undefined
...
- **First genuine Stage-5 STAAD benchmark completed (19 Sep 2026):** STAAD.Pro CONNECT 22.09.00.115 returned 63 primary cases and 141 combinations for the 540-node / 749-member final-service model. All 204 result sets and 1,029,792 detailed numerical comparisons pass the current 2% plus absolute tolerance policy; all 203 ULS/SLS combination-envelope checks and all 28 LM1 governing-envelope checks also pass, with the same governing combination/source case recovered in every envelope item. Sanitized evidence is stored in `verification_evidence/staad_stage5_2026-09-19.json`; the 36 MB raw ANL is identified by SHA-256 rather than committed
...
4. **Independent structural validation — in progress.** STAAD `.ANL` import now reconstructs the same governing LM1 envelope quantities used by the application and separates them from lower-level raw result diagnostics. Continue with genuine installed-software runs, reconcile any remaining axes/signs/units/model assumptions, establish justified tolerances across additional bridge cases and close clause/project acceptance gaps.
...
1. **Batch 6 — close independent structural verification and engineering QA (in progress).** The application parses the full STAAD result catalogue once, indexes primary cases/combinations for reuse, applies a shared relative-plus-absolute tolerance policy, and reports import coverage, detailed numerical agreement and governing-envelope agreement separately from engineering acceptance. The first genuine Stage-5 STAAD benchmark is now numerically closed. Remaining work is to confirm exact exported-file/source identity for that run, complete the engineering-review record, exercise the remaining benchmark configurations (including MIDAS where applicable), justify final project tolerances and close outstanding clause/project acceptance checks.
2. **Batch 7 — harden and release deterministic application v1.0.** Finish end-to-end workflow testing, any GUI consistency issues revealed by real use, remaining performance issues, validation/error handling, report pagination and polish, project-file compatibility, documentation/help, EXE versioning and the final Windows release build.
