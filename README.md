...
1. **Batch 6 — close independent structural verification and engineering QA (in progress).** The genuine Stage-5 STAAD benchmark, engineering source/model review, finalized v1 tolerances and the analysis-vs-design acceptance matrix are implemented. Independent published code verification has also begun: one JRC rectangular-flexure case and one JRC vertical-link-spacing case now pass as scoped supporting evidence. The research manifest deliberately remains locked for LM1 code interpretation, EN 1990 combinations and the broad flexure/shear/torsion/cracking/deflection/fatigue/detailing milestones until representative independent evidence is complete; construction-stage and local-deck structural checks also remain external-verification items. MIDAS cross-verification is deferred to V2.
